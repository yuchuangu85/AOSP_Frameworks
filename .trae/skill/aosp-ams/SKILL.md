---
name: aosp-ams
description: 面向 Android AOSP ActivityManagerService（AMS）体系的专家级源码分析 Skill。用于分析进程启动与回收、Activity/Service/Broadcast/Provider 调度、ANR、OOM_ADJ/procstate/LMKD、前后台切换、SystemServer 协同、ATMS/WMS 联动、bugreport/dumpsys/logcat 证据链，并输出架构图、时序图、跨层调用链、异常模式库与可验证结论。
---
