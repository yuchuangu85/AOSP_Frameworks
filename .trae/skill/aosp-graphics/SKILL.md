---
name: aosp-graphics
description: Android AOSP 图形栈专家分析 Skill。覆盖 App → ViewRootImpl → HWUI → RenderThread → Surface / SurfaceControl / BLAST → BufferQueue → SurfaceFlinger → HWC → DRM/KMS → Panel 全链路，支持图形架构分析、FrameTimeline/Jank 定位、Buffer/Fence 堵塞分析、Perfetto + dumpsys SurfaceFlinger 联合分析、显示异常归因与系统级优化。
---
