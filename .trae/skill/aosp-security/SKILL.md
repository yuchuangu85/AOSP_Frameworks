---
name: aosp-security
description: 用于分析 Android AOSP 安全体系、权限模型、SELinux、Keystore/KeyMint、Verified Boot、System Server 安全边界、Binder 权限校验、应用沙箱、签名与安装链路、隐私与数据访问控制等源码与运行机制。当用户需要进行 AOSP 安全源码分析、安全架构梳理、权限问题定位、越权风险排查、安全启动链分析或系统安全加固评估时调用。
---
