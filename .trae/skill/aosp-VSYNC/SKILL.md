---
name: aosp-vsync
description: 面向 Android / AOSP 显示时序与 VSYNC 全链路分析的专家级 Skill。用于分析 App 卡顿、丢帧、时序漂移、输入到显示延迟、Choreographer 调度异常、SurfaceFlinger 合成节奏异常、HWC/Display VSYNC 不稳定、FrameTimeline/Jank 根因定位。输出必须覆盖架构设计思想、跨层调用链、关键源码机制、时序图、日志/trace证据、异常模式匹配与优化建议。
---
