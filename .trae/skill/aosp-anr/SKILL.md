---
name: aosp-anr
description: 面向 Android Open Source Project（AOSP）的系统级 ANR（Application Not Responding）源码与运行时联合分析 Skill。用于分析输入超时、Broadcast 超时、Service 超时、ContentProvider 超时、前台服务超时，以及由锁竞争、Binder 阻塞、主线程阻塞、System Server 拥塞、调度延迟、I/O 卡死、图形卡顿级联等引发的 ANR。要求基于源码调用链、线程栈、trace、logcat、bugreport、dumpsys、Perfetto 等证据建立可验证根因结论，而非停留在经验猜测。
---
