---
name: aosp-binder
description: 用于系统级分析 Android Binder 机制与相关问题，覆盖 Java Binder / native Binder / ServiceManager / system_server / app 进程 / Binder Driver 跨层调用链。适用于分析 Binder 调用流程、同步/异步事务、线程池阻塞、Binder ANR、死锁、耗时抖动、system_server 卡顿、跨进程通信性能问题，以及基于源码、trace、dumpsys、binder state、kernel 日志进行根因定位与优化设计。
---
