---
name: aosp-power
description: 用于分析 Android AOSP 电源管理体系（PowerManagerService / DisplayPower / Power HAL / Suspend / WakeLock / Doze / App Standby / Battery Saver / Thermal / Charging / Kernel Power）源码、运行机制、跨层调用链、时序关系与异常根因。适用于分析亮灭屏、休眠唤醒、耗电异常、无法休眠、频繁唤醒、WakeLock泄漏、Doze失效、待机耗电、热功耗联动、充电状态联动、电源策略冲突等问题。输出必须基于源码、日志、dumpsys、trace、kernel节点与可验证证据，禁止无依据猜测。
---
