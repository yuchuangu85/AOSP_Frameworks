---
name: aosp-media
description: 面向 Android/AOSP Media 体系的源码分析 Skill。用于分析 MediaPlayer / MediaCodec / MediaExtractor / NuPlayer / AwesomePlayer（历史）/ Stagefright / Codec2 / OMX（历史兼容）/ AudioTrack / AudioFlinger / AudioPolicy / Camera 与 Media 交互 / Surface 与 BufferQueue 视频输出链路 / DRM / AV sync / offload / tunneled playback / media server 相关机制。适用于播放异常、编解码失败、黑屏有声/有图无声、音视频不同步、首帧慢、seek 卡顿、录制异常、media server 崩溃、binder 调用链、native framework 设计分析与性能优化。
---
