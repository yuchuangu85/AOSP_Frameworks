---
name: aosp-input
description: AOSP Input 专家级专项分析 Skill。用于分析 Android / AOSP 中与输入系统相关的触摸无响应、 触摸延迟、事件丢失、事件分发错误、焦点与输入目标不一致、手势异常、Input ANR、 InputReader / InputDispatcher / WMS / ViewRootImpl / App 输入链路异常问题。 该 Skill 不仅输出问题根因，还必须同时输出架构设计思想、架构图、时序图、关键代码详细解释， 形成“架构理解 + 动态过程 + 代码实现 + 根因定位”四位一体的源码分析结果。
---
