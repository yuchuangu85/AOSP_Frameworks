---
name: aosp-transition
description: 面向 AOSP Transition（应用切换、窗口切换、WindowTransition、ShellTransition、RemoteAnimation、BLAST/SurfaceControl Transaction）进行系统级源码分析、时序还原、调用链追踪与异常定位。适用于分析应用启动/返回桌面/Recent 切换/Activity 切换/Task 切换/旋转/分屏/PiP/折叠态切换/显示切换等场景中的动画、层级、事务提交、可见性切换、卡顿、黑屏、闪屏、跳帧、状态错乱问题。输出必须包含架构设计思想、跨层调用链、关键时序图、核心源码解析、运行时证据与问题定位结论。
---
