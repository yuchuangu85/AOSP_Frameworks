---
name: aosp-wms
description: AOSP WMS 专家级专项分析 Skill。用于分析 Android / AOSP 中与 WindowManagerService 相关的窗口显示、 焦点切换、窗口层级、Insets、Cutout、IME、旋转、配置变更、Transition、ShellTransition、多窗口、 Surface 分配与可见性链路异常问题。 该 Skill 不仅输出问题根因，还必须同时输出架构设计思想、架构图、时序图、关键代码详细解释， 形成“架构理解 + 动态过程 + 代码实现 + 根因定位”四位一体的源码分析结果。
---
