---
name: aosp-camera
description: 用于执行 Android AOSP Camera 子系统源码分析、架构建模、调用链追踪、时序还原、性能与稳定性问题定位。覆盖 App / Camera API / CameraService / CameraProvider / HAL3 / RequestThread / BufferQueue / Surface / 图像流转 / Session 配置 / 3A / Reprocess / OfflineSession / 多摄 / 扩展能力等核心路径。适用于分析拍照、预览、录像、切摄像头、切模式、流配置失败、首帧慢、黑屏、卡住、花屏、拍照慢、录像异常、元数据异常、HAL 交互异常等问题。明确排除 ANR 专项分析；当问题核心属于 ANR 时，应交由独立 ANR Skill 处理。
---
