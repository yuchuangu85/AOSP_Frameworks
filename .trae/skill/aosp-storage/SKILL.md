---
name: aosp-storage
description: AOSP 存储栈源码分析专家技能。用于分析 Android 存储子系统的整体架构、核心服务、 挂载流程、用户态与内核态协作、应用存储访问路径、外部存储与分区卷管理、FUSE/sdcard、 vold / installd / StorageManagerService / fs_mgr / vold daemon / quota / adoptable storage / encrypted storage 等关键机制。适用于源码阅读、系统行为解释、挂载失败、存储权限异常、 容量统计异常、I/O 性能问题、媒体扫描异常、应用安装与数据目录问题、U 盘/SD 卡问题、 多用户存储隔离、文件可见性与访问链路分析。
---
